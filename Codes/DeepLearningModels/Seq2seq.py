...
class Seq2Seq(nn.Module):
    ...
    def __init__(self,
                 ...
                 external_input_encod_ind: bool=False,
        ...
        Args:
            ...
            external_input_ind: bool, optional, default=False
                If true, there are extra control inputs (not predicted at output)
            external_input_encod_ind: bool, optional, default=False
                If true, use control inputs as encoder inputs
        '''
        ...
        self.H_out = encoder_hidden_size
        self.num_layers = num_layers
        self.dropout=dropout
        self.D=bi_direct
        self.external_input_ind=external_input_ind
        self.external_input_encod_ind=external_input_encod_ind
        self.alpha = alpha

        # define the number of input feauters for encoder
        if external_input_encod_ind:
            num_features_encoder = num_features
        else:
            num_features_encoder = num_features-self.num_external_inputs
            num_features_decoder = num_features


        # Encoder
        self.encoder = Encoder(model_struct=model_struct,
                               num_features=num_features_encoder,
        ...
        # Decoder
        self.decoder = Decoder(model_struct=model_struct,
                               num_features=num_features_decoder,
    ...
    def one_step_predict(self, x, y, teacher_forcing = True, teacher_forcing_ratio=0.0):
        ...
        x_input = create_glc_afeter_feed(s=x)

        # split x into input for encoder and one for decoder
        encoder_input, decoder_input = torch.split(x_input, [x_input.size(1)-1, 1], dim=1)


        # remove extra features for encoder
        if not self.external_input_encod_ind:
            encoder_input = encoder_input[:,:,:encoder_input.size(2)-self.num_external_inputs]

        # get context vector from encoder
        encoder_out, encoder_hidden = self.encoder(encoder_input)
    ...
    def train_BO(self, dataset, label_indices: list, 
        ...
        for epoch in range(1, epochs+1):
            ...
            if epoch%10==0 and not kfold_ind:
                # train_loss_val = np.mean(batch_original_losses)#self.evaluate(train_data_loader, train_lengths)
